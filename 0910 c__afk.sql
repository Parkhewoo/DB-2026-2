...
--가상컬럼명을 설정
--제품테이블에서 제조업체의 중복데이터를 제거하시오 (단 제조업체를 대표업체로 컬럼명 변경)
...
--where(조건절)
--제품테이블에서 제조업체가 한빛제과인데 데이터행을 출력하시오
select 제품명, 단가, 제조업체 
    from 제품
    where 제조업체='한빛제과';
...
--주문테이블에서 주문고객이 carrot 이면서 수량이 15개이상인 주문한 주문제품, 수량, 주문일자를 출력하시오
...
--고객 테이블에서 나이가 20세 이상 30세 이하인 고객의 고객이름, 나이, 직업을 출력하시오
select 고객이름, 나이, 직업
    from 고객
...
select * from 고객;
--고객테이블에서 고객이름에 용글자가 포함된 고객이름, 나이를 출력하시오
select 고객이름, 나이
    from 고객
    where 고객이름 like '%용%';
